...
class AgendamentoPageService {
  static const String _baseUrl =
      'https://${ApiConfig.baseUrl}/api/usuario/homepage/agendamento';

  final http.Client client;

  AgendamentoPageService({http.Client? client})
    : client = client ?? http.Client();

  Future<List<DateTime>> listarHorariosIndisponiveis(int servicoId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/$servicoId'));
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Falha ao carregar horários. Status: ${response.statusCode}',
        );
      }
    } catch (e) {
