...
public class MinhasSoliticacoesController {
    ...
    private AgendamentoService agendamentoService;
        ...
        @Operation(
            ...
            description = "Retorna uma lista de solicitações (agendamentos) feitas pelo usuário informado.",
            tags = { "Solicitações enviadas pelo usuario" }
        )
        @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista de agendamentos retornada com sucesso",
                content = @Content(mediaType = "application/json",
                    schema = @Schema(implementation = AgendamentoMinhasSolicitacoesDTO.class))),
            @ApiResponse(responseCode = "404", description = "Usuário não encontrado")
        })
        @GetMapping("/{usuarioId}")
        public ResponseEntity<List<AgendamentoMinhasSolicitacoesDTO>> carregaAgendamentos(
                @Parameter(description = "ID do usuário para buscar as solicitações", required = true, example = "1")
        ...
        @Operation(
            summary = "Busca detalhes de um agendamento específico",
            description = "Retorna os detalhes de uma solicitação (agendamento) específica pelo seu ID.",
            tags = { "Solicitações enviadas pelo usuario" }
        )
        @ApiResponses(value = {
        ...
        public ResponseEntity<AgendamentoMinhasSolicitacoesDTO> carregaAgendamento(
                @Parameter(description = "ID do agendamento para buscar os detalhes", required = true, example = "1")
